...
function ledgerRow(label, value, sub, opts = {}) {
  ...
  if (opts.stack) {
    ...
    return h(
      "div",
      { display: "flex", flexDirection: "column", borderBottom: `1px solid ${RULE}`, padding: "10px 2px 12px" },
...
export function build(metrics) {
  ...
  return h(
    "div",
    { display: "flex", flexDirection: "column", width: "100%", height: "100%", backgroundColor: PAPER, color: INK, padding: "34px 56px 26px" },
    // masthead with double rule
    h(
      "div",
      { display: "flex", flexDirection: "column", borderBottom: `3px solid ${INK}`, paddingBottom: 10 },
      h(
        "div",
        { display: "flex", justifyContent: "space-between", alignItems: "baseline" },
        h("div", { display: "flex", fontSize: 40, fontWeight: 800, fontFamily: "Spectral", letterSpacing: 1 }, "The Record Book"),
        h("div", { display: "flex", fontSize: 17, fontFamily: "JetBrains Mono", color: FAINT, letterSpacing: 2 }, `${s.since} — ${s.until}`),
      ),
      h("div", { display: "flex", fontSize: 15, fontFamily: "JetBrains Mono", letterSpacing: 4, color: FAINT, marginTop: 4 }, "TRACKRECORD · A LEDGER OF SHIPPED WORK"),
    ),
    h("div", { display: "flex", borderBottom: `1px solid ${INK}`, marginTop: 3 }),
    // hero band: LOC + PRs side by side, serif at full size
    h(
      "div",
      { display: "flex", alignItems: "baseline", justifyContent: "space-between", padding: "22px 2px 18px", borderBottom: `1px solid ${RULE}` },
      h(
        "div",
        { display: "flex", flexDirection: "column" },
        h("div", { display: "flex", fontSize: 18, letterSpacing: 3, color: FAINT, fontFamily: "JetBrains Mono" }, "LINES OF CODE ADDED"),
        h("div", { display: "flex", fontSize: 124, fontWeight: 800, color: OXBLOOD, fontFamily: "Spectral", lineHeight: 1 }, s.loc),
      ),
      h(
        "div",
        { display: "flex", flexDirection: "column", alignItems: "flex-end" },
        h("div", { display: "flex", fontSize: 18, letterSpacing: 3, color: FAINT, fontFamily: "JetBrains Mono" }, "PULL REQUESTS SHIPPED"),
        h("div", { display: "flex", fontSize: 124, fontWeight: 800, fontFamily: "Spectral", lineHeight: 1 }, s.prs),
      ),
    ),
